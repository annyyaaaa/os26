...
{   
    ...
    n = msgrcv(msg_id, &message, sizeof(message.data), message.mtype, 0);
    int er_num=errno;     
    ...
    if (n>0)
    {   
        printf("Получено от PID %d: %s\n", message.data.client_pid, message.data.text);

        if (write(1, message.data.text, n) != n)
        {
